...
import { fetchRevenueOverView_admin } from '../../api/revenue';
...
const Revenue = props => {
    ...
    const [graphModeDisplayName,setGraphModeDisplayName] = useState('This Month')
    const [dataTime,setDataTime] = useState(9) 
    const [leadData,setLeadData] = useState([]);  
    const [loadingLeadData,setLoadingLeadData] = useState(true); 
    
    useEffect(() => {
        fetchRevenueOverView_admin(graphMode,dataTime,(response)=>AnaylictsApiCallBack(response,graphMode,{setLeadData,setLoadingLeadData}));
    },[graphMode])
            ...
            <div className="col-12 col-lg-12">
                <div className="card radius-15 overflow-hidden">
                    <div className="card-body"> 
                        ...
                        <Chart height={300} chartType="bar" chart_counter={2} data={leadData}  /> 
                    </div>
                </div>
            </div>
        </div>
